...
exports.addUser = ( value ) => {
  let _sql = "insert into users set name=?,password=?,avator=?,moment=?;"
...
exports.addArticle = ( value ) => {
  let _sql = "insert into articles set name=?,title=?,content=?,md=?,userid=?,moment=?,avator=?;"
  return query( _sql, value )
}
// 增加文章评论数
...
exports.addComment = ( value ) => {
  let _sql = "insert into comments set name=?,content=?,moment=?,articleid=?,avator=?;"
  return query( _sql, value )
}
// 通过名字查找用户
...
// 通过名字查找用户数量判断是否已经存在
exports.searchUserCountByName =  ( name ) => {
  let _sql = `select count(name) as count from users where name="${name}";`
  return query( _sql)
}
// 通过用户的名字查找文章
exports.searchArticleByUser =  ( name ) => {
  let _sql = `select name from articles where name="${name}";`
  return query( _sql)
}
// 通过文章id查找
exports.searchByArticleId =  ( id ) => {
  let _sql = `select id from articles where id="${id}";`
  return query( _sql)
}
// 通过articleid查找
exports.searchCommentByArticleId =  ( id ) => {
  let _sql = `select * from comments where articleid="${id}";`
  return query( _sql)
}
...
// 查询分页文章
exports.searchArticleByPage = ( page ) => {
...
// 查询个人分页文章
exports.searchArticleByUserPage = (name,page) => {
  let _sql = ` select * from articles where name="${name}" order by id desc limit ${(page-1)*10},10 ;`
  return query( _sql)
}
// 更新修改文章
...
// }
// 评论分页
exports.searchCommentByPage = (page,articleid) => {
